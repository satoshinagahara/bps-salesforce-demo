...
1. **1-step で十分なら 1-step**: 余計に分解しない
2. **Executor は狭い文脈で動く**: 1 step = 1 成果物 = 1 意図
3. **F パターンを要するタスク**: Phase α-3 では escalate モードが使えないので、`mode="full"` で出し `success_criteria` に「判定根拠の提示」を含める。最終判断は synthesis で補強する
4. **データ欠落の検出**: ユーザ発話から elementary_id が一意に特定できないときは単一の full step でユーザ意図を executor に丸投げする
