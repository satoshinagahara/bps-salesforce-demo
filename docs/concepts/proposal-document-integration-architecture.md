...
2. **OpportunitySummaryBatch拡張**: サマリカード生成時に `Proposal_Context__c` のテキストを入力ソースに追加
3. **品質比較**: 提案書ありサマリカード vs なしサマリカードの品質差を定量評価
4. **LWCファイル投入UI**: 商談ページからPPTXをドラッグ&ドロップ→Lambda呼び出し→結果保存のフロー実装
